import{r as h}from"./index.DiEladB3.js";const _="modulepreload",R=function(t){return"/"+t},N={},$=function(s,n,d){let l=Promise.resolve();if(n&&n.length>0){let c=function(m){return Promise.all(m.map(u=>Promise.resolve(u).then(g=>({status:"fulfilled",value:g}),g=>({status:"rejected",reason:g}))))};document.getElementsByTagName("link");const a=document.querySelector("meta[property=csp-nonce]"),b=a?.nonce||a?.getAttribute("nonce");l=c(n.map(m=>{if(m=R(m),m in N)return;N[m]=!0;const u=m.endsWith(".css"),g=u?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${m}"]${g}`))return;const p=document.createElement("link");if(p.rel=u?"stylesheet":_,u||(p.as="script"),p.crossOrigin="",p.href=m,b&&p.setAttribute("nonce",b),document.head.appendChild(p),u)return new Promise((i,o)=>{p.addEventListener("load",i),p.addEventListener("error",()=>o(new Error(`Unable to preload CSS for ${m}`)))})}))}function r(c){const a=new Event("vite:preloadError",{cancelable:!0});if(a.payload=c,window.dispatchEvent(a),!a.defaultPrevented)throw c}return l.then(c=>{for(const a of c||[])a.status==="rejected"&&r(a.reason);return s().catch(r)})};var j={exports:{}},v={};/**
 * @license React
